...
class CollisionDetection {
  ...
  */
  template<typename T> bool isTraversable(const T* node) const {
    ...
    */
    ...
    // 2D collision test
    if (t == 99) {
      const int index = node->getIdx();
      return grid != nullptr && index >= 0 &&
             static_cast<std::size_t>(index) < grid->data.size() &&
             !grid->data[index];
    }
  ...
  */
  float configurationCost(float, float, float) const {return 0;}
  ...
  */
  void updateGrid(const GridMap* map) { grid = map; }
