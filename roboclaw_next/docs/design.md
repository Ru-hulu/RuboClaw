...
每个 Tool 都需要提供稳定且含义明确的工具名称、功能描述和输入参数约束，并返回能够表达执行结果的内容。工具描述不仅用于说明其功能，也用于帮助 LLM 判断何时调用该工具；输入参数则通过 JSON Schema 明确字段类型、必填项和取值范围。

MCP Tool Definition 是工具契约的主要来源。RoboClaw 通过 `MCPToolAdapter` 将其转换为 Agent Runtime 可以管理的工具对象，不再为同一个工具重复维护另一套参数定义。工具执行结果应至少能够区分成功与失败，并在必要时保留结构化信息，供后续模型调用或系统模块继续使用。

### 2. 工具失败处理

工具调用失败不应直接中断整个 Agent Loop。Runtime 应将参数错误、连接异常或工具内部异常转换为对应的工具结果，并作为 `role="tool"` 的消息写回 Session。这样既能保证 Assistant Tool Call 与 Tool Message 的配对关系，也能让 LLM 根据失败原因调整参数、选择其他工具，或者向用户说明当前任务无法继续。

第一版只需要保留清晰的错误信息和错误状态，不引入复杂的重试、错误分类和恢复策略。工具是否可以重试以及如何恢复，应在真实能力接入后根据具体场景逐步确定。

### 3. 短时工具与长时任务

当前 Tool 调用采用同步等待方式：Runtime 发起调用后，等待工具执行完成，再将结果返回给 LLM。这种方式适合状态查询、简单计算和执行时间较短的机器人动作，也有利于保持初期架构清晰。

导航、自动校准和数据采集等操作可能持续较长时间。第一版仍可在执行时间可控的前提下沿用同步调用；当长时间阻塞、状态查询或用户中止成为实际需求时，再将此类能力拆分为启动、状态查询和停止等工具。当前阶段不提前引入通用异步任务系统。
